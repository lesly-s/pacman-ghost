...
def joinFactors(factors: List[Factor]):
    ...
    f = Factor(unconditioned, conditioned, varDomainsDict)

    for possible in f.getAllPossibleAssignmentDicts():
        p = 1
        for factor in factors:
            p = p * factor.getProbability(possible)
        f.setProbability(possible, p)
...
def eliminateWithCallTracking(callTrackingList=None):
    ...
    def eliminate(factor: Factor, eliminationVariable: str):
        ...
        f = Factor(unconditioned, conditioned, factor.variableDomainsDict())

        for possible in f.getAllPossibleAssignmentDicts():
            p = 0
            for var in factor.variableDomainsDict()[eliminationVariable]:
                possible[eliminationVariable] = var
                p = p + factor.getProbability(possible)
            f.setProbability(possible, p)
        return f
