...
def constructBayesNet(gameState: hunters.GameState):
    ...
    observ = set()
    for n in range(0, MAX_NOISE + 1):
        for pac_p in pos:
            for g_p in pos:
                mandist = manhattanDistance(pac_p, g_p)
                if (mandist - n) > 0:
                    observ.add(mandist - n)
                observ.add(mandist + n)

    observ = list(observ)
    variableDomainsDict[OBS0] = observ
    variableDomainsDict[OBS1] = observ
    "*** END YOUR CODE HERE ***"
...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet: bn, queryVariables: List[str], evidenceDict: Dict, eliminationOrder: List[str]):
        ...
        for var in eliminationOrder:
            cpts, joined = joinFactorsByVariable(cpts, var)
            if len(joined.unconditionedVariables()) > 1:
                cpts.append(eliminate(joined, var))

        normalized = normalize(joinFactors(cpts))
        return normalized
        "*** END YOUR CODE HERE ***"
    ...
    def elapseTime(self, gameState: busters.GameState):
        ...
        for pos in self.allPositions:
            ...
            for p in self.allPositions:
                self.beliefs[p] = self.beliefs[p] + past[pos] * newdist[p]
        "*** END YOUR CODE HERE ***"
...
class ParticleFilter(InferenceModule):
    ...
    def getBeliefDistribution(self):
        ...
        for particle in self.particles:
            if particle not in places:
                places[particle] = 0
            places[particle] = places[particle] + 1

        dbution = DiscreteDistribution(places)
        dbution.normalize()
        return dbution
        "*** END YOUR CODE HERE ***"
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        "*** YOUR CODE HERE ***"
        #raiseNotDefined()
        dbuition = DiscreteDistribution()
        pac_pos = gameState.getPacmanPosition()
        jail_pos = self.getJailPosition()

        for particle in self.particles:
            dbuition[particle] = dbuition[particle] + self.getObservationProb(observation, pac_pos, particle, jail_pos)

        if dbuition.total() != 0:
            dbuition.normalize()
            new = []
            for particle in self.particles:
                new.append(dbuition.sample())
            self.particles = new
        else:
            self.initializeUniformly(gameState)
        "*** END YOUR CODE HERE ***"
    ...
    def elapseTime(self, gameState):
        ...
        for particle in self.particles:
            pos = self.getPositionDistribution(gameState, particle)
            next_state.append(pos.sample())
