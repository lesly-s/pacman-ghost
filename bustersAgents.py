...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState: busters.GameState):
        ...
        for ghost in livingGhostPositionDistributions:
            highest = max(ghost, key=ghost.get)
            dist = self.distancer.getDistance(highest, pacmanPosition)
            if min_dist > dist:
                nearest_ghost = highest
                min_dist = dist

        bestest_dist = float('inf')
        bestest = None

        for legal in gameState.getLegalPacmanActions():
            succ_pos = Actions.getSuccessor(pacmanPosition, legal)
            succ_dist = self.distancer.getDistance(nearest_ghost, succ_pos)
            if bestest_dist > succ_dist:
                bestest = legal
                bestest_dist = succ_dist
        return bestest
